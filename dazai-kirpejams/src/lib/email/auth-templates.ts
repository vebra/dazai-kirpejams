...
// ============================================
// EL. PAŠTO PATVIRTINIMO PRIMINIMAS — siunčiama admin'o iš /admin/verifikacija
// vartotojams, kurie užsiregistravo + patvirtinti, bet niekada nepaspaudė
// patvirtinimo nuorodos (auth.users.email_confirmed_at = null).
// ============================================

export type EmailConfirmReminderInput = {
  firstName: string
  /** Supabase magiclink action_link — paspaudus patvirtina el. paštą ir prijungia. */
  confirmUrl: string
  lang: 'lt' | 'en' | 'ru'
}

const EMAIL_CONFIRM_REMINDER_COPY = {
  lt: {
    ...
    badge: 'Priminimas',
    title: (name: string) =>
      name ? `Sveiki, ${name}` : 'Patvirtinkite el. paštą',
    intro:
    ...
    validity:
      'Nuoroda galioja ribotą laiką. Jei nebeveiks — parašykite mums ir atsiųsime naują.',
  ...
  en: {
    ...
    badge: 'Reminder',
    title: (name: string) => (name ? `Hello, ${name}` : 'Confirm your email'),
    intro:
    ...
    validity:
      'The link is valid for a limited time. If it no longer works — write to us and we will send a new one.',
  ...
  ru: {
    ...
    badge: 'Напоминание',
    title: (name: string) =>
      name ? `Здравствуйте, ${name}` : 'Подтвердите эл. почту',
    intro:
    ...
    validity:
      'Ссылка действительна ограниченное время. Если она не работает — напишите нам, и мы отправим новую.',
...
export function buildEmailConfirmReminderEmail(
  input: EmailConfirmReminderInput
): { subject: string; html: string; text: string } {
  const c =
    EMAIL_CONFIRM_REMINDER_COPY[input.lang] ?? EMAIL_CONFIRM_REMINDER_COPY.lt
  const safeName = escapeHtml(input.firstName || '')
  ...
  <tr>
    <td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:16px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:24px 32px 0;">
            <a href="${input.confirmUrl}" style="display:inline-block;padding:14px 28px;background:${BRAND_MAGENTA};color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;border-radius:10px;">
              ${escapeHtml(c.ctaLabel)} →
            </a>
            <p style="margin:12px 0 0;font-size:12px;color:${GRAY_500};">
              ${escapeHtml(c.validity)}
