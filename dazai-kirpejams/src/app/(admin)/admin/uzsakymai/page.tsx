    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex-shrink-0 flex items-center gap-2">
          <Link
            href="/admin/uzsakymai/naujas"
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-brand-magenta hover:bg-brand-magenta-dark text-white rounded-lg text-[13px] font-semibold transition-colors"
          >
            + Naujas užsakymas
          </Link>
          <a
            href={`/admin/uzsakymai/eksportas${status ? `?status=${status}` : ''}${dateFrom ? `${status ? '&' : '?'}from=${dateFrom}` : ''}${dateTo ? `${status || dateFrom ? '&' : '?'}to=${dateTo}` : ''}`}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#F5F5F7] hover:bg-[#e8e8ec] border border-[#ddd] rounded-lg text-[13px] font-semibold text-brand-gray-900 transition-colors"
            download
          >
            CSV eksportas
          </a>
