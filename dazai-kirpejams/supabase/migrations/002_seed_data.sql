...
insert into products (
  ...
  ('color-shock-1-0', 'CS-1.0', (select id from cat), (select id from brd),
    'Color SHOCK 1.0 — Juoda', 'Color SHOCK 1.0 — Black', 'Color SHOCK 1.0 — Чёрный',
  ...
  ('color-shock-4-0', 'CS-4.0', (select id from cat), (select id from brd),
    'Color SHOCK 4.0 — Vidutinė ruda', 'Color SHOCK 4.0 — Medium Brown', 'Color SHOCK 4.0 — Средне-коричневый',
  ...
  ('color-shock-6-0', 'CS-6.0', (select id from cat), (select id from brd),
    'Color SHOCK 6.0 — Tamsiai šviesi', 'Color SHOCK 6.0 — Dark Blonde', 'Color SHOCK 6.0 — Тёмный блонд',
  ...
  ('color-shock-6-4', 'CS-6.4', (select id from cat), (select id from brd),
    'Color SHOCK 6.4 — Šviesiai varinė', 'Color SHOCK 6.4 — Light Copper', 'Color SHOCK 6.4 — Светлая медь',
  ...
  ('color-shock-8-0', 'CS-8.0', (select id from cat), (select id from brd),
    'Color SHOCK 8.0 — Šviesi', 'Color SHOCK 8.0 — Light Blonde', 'Color SHOCK 8.0 — Светлый блонд',
  ...
  ('color-shock-9-1', 'CS-9.1', (select id from cat), (select id from brd),
    'Color SHOCK 9.1 — Šalta šviesi', 'Color SHOCK 9.1 — Cool Blonde', 'Color SHOCK 9.1 — Холодный блонд',
...
insert into products (
  ...
  ('oxidant-3', 'OX-3', (select id from cat), (select id from brd),
    'Oksidantas 3%', 'Oxidant 3%', 'Оксидант 3%',
    'Profesionalus 3% oksidantas toninimui ir tamsinimui.',
    'Professional 3% oxidant for toning and darkening.',
    'Профессиональный 3% оксидант для тонирования и затемнения.',
    890, 1000, 50, true),

  ('oxidant-6', 'OX-6', (select id from cat), (select id from brd),
    'Oksidantas 6%', 'Oxidant 6%', 'Оксидант 6%',
    'Profesionalus 6% oksidantas dažymui iki 2 tonų.',
    'Professional 6% oxidant for coloring up to 2 tones.',
    'Профессиональный 6% оксидант для окрашивания до 2 тонов.',
    890, 1000, 50, false),

  ('oxidant-9', 'OX-9', (select id from cat), (select id from brd),
    'Oksidantas 9%', 'Oxidant 9%', 'Оксидант 9%',
    'Profesionalus 9% oksidantas šviesinimui iki 3 tonų.',
    'Professional 9% oxidant for lightening up to 3 tones.',
    'Профессиональный 9% оксидант для осветления до 3 тонов.',
    890, 1000, 50, false)
on conflict (slug) do nothing;
